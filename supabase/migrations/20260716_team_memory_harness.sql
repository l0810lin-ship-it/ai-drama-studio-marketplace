...
begin
  ...
  plain_code := encode(gen_random_bytes(18), 'hex');
  insert into public.drama_invites(organization_id, code_hash, role, created_by, expires_at)
  values (target_organization, encode(digest(plain_code, 'sha256'), 'hex'), invite_role, actor, now() + make_interval(hours => valid_hours));
  return plain_code;
end; $$;
...
begin
  ...
  where code_hash = encode(digest(invite_code, 'sha256'), 'hex') for update;
